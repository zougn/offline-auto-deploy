...
function retrieveAndDeepTree(dep) {
  if (dep) {
    for ([name, version] of Object.entries(dep)) {
      ...
      if (CONFIG.cache1.has(`${name}${version}`)) {
        continue
      }else{
        CONFIG.cache1.add(`${name}${version}`);
      }
      
      const pkgInfo = fetchPackageInfo(name);
      version = getMaxSatisfyingVersion(pkgInfo, version);
      if (CONFIG.cache2.has(`${name}@${version}`)) {
        continue
      }else{
...
function main() {
  ...
  const pkgInfo = fetchPackageInfo('@antv/g6');
  let version = getMaxSatisfyingVersion(pkgInfo, '^5.0.45');
