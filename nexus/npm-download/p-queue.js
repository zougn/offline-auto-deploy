...
async function retrieveAndDeepTree(dep) {
  ...
  await Promise.all(Object.entries(dep).map(async ([name, versionRange]) => {
...
async function saveToYAML(pkgData) {
  ...
  try {
    ...
    console.error(`保存失败: ${pkgData.name}@${pkgData.version}`, error.message);
  }
}
...
async function main() {
    try {
        ...
        queue.onIdle().then(() => {
            ...
            console.log(`🌐 唯一下载地址 ${CONFIG.urls.size} 个`);
        });
    } catch (error) {
